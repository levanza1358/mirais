...
GitHub Copilot account setup uses **Login with GitHub**: Mirais opens GitHub's official browser login/device flow and polls the isolated local sidecar until authenticated. The dashboard never renders a GitHub email or password input.

The Providers page also shows a rolling provider-health panel with request count, error rate, failed requests, and average latency. The **Audit log** page records safe configuration changes, and **Capabilities** shows synced model context/output limits and capability badges (tools, vision, reasoning, JSON, and similar metadata).
...
- **Accounts card:** rows with status dot, label, masked key (`sk••••xxxx`), created time, enable switch, remove (confirm modal). GitHub Copilot accounts are added individually with a required local sidecar base URL (for example `http://127.0.0.1:4141/v1`) and an optional sidecar key; the account row displays that endpoint. Each sidecar is isolated to one entitled GitHub account. A **Delete all** action appears when accounts exist and requires a destructive confirmation before deleting every account for that provider. It removes accounts one at a time and displays a non-dismissible live progress bar with deleted/failed counts. **Add account** opens a modal with choices: *Login with ChatGPT* (ChatGPT provider and legacy OpenAI providers only — starts the Codex OAuth PKCE flow, opens the OpenAI login page in a new tab, waits with a spinner until the account appears; labeled `ChatGPT (email)`), *Paste Codex JSON* (Codex provider only — imports `accessToken` and `refreshToken` locally), *Single API key* (label + key with show/hide toggle), or *Bulk API keys* (textarea, one key per line — labels auto-generated, duplicates skipped, reports added/skipped counts). Accounts are fully addable/removable. The list is **paginated at 20 per page** with Prev/Next controls and a "1–20 of N · Page X / Y" indicator. Each row also shows **per-account usage** (`N req · M tok today`, hover for all-time totals + cost) aggregated from request logs (auto-refreshes every 30s). ChatGPT/Codex OAuth rows and GitHub Copilot rows show the current remaining quota percentage, usage bar, and reset time inline from their live provider quota APIs (auto-refreshes every 60s); the Codex quota button opens its detailed breakdown.
- **Models card:** active/disabled counts, **Fetch models** button (pulls the full model list from the provider's `/models` API; auto-fetches once when the provider has an enabled account but zero models), add-model input, model chips with enable toggle (line-through when disabled) and remove. Each chip shows **context length** (`128k ctx`), **max output** (`16k out`), and **capability badges** (Reasoning / Vision / PDF / Tools / JSON) captured during sync, with a full tooltip on hover. Codex models with a paid ChatGPT requirement show a crown badge (for example, `Plus / Pro` for GPT-5.6 Sol and `Pro` for Codex Spark); it is informational and the signed-in account's live catalog remains authoritative. Each chip has a **test button** (⚡) that sends a tiny chat completion to the upstream and shows the result inline (✓ latency / ✗ error, hover for detail). **Test all** opens a choice modal: *Test all models only* or *Test all & delete error models* (failing models are removed after the run). Removing a single model chip asks for confirmation first; **Delete all** requires a destructive confirmation. Sync never deletes previously synced models unless the *Prune removed models* setting is enabled.
...
### 3.5 Combos (`/combos`)
- Summary bar shows total combos, total targets, and combos needing attention.
- Search by combo name and a **Problems only** filter make large fallback libraries easy to scan.
- Each combo card renders a vertical chain with numbered steps, `fallback` connectors, live healthy-account counts (`healthy/total`), disabled/missing model warnings, strategy badge, and last diagnostic result.
- Copy button copies the client model id (`combo:<name>`); **Test all** probes every combo sequentially and keeps per-combo diagnostics visible.
- **Editor modal:** name + strategy selector (`sequential` or `round_robin`) + drag-and-drop sortable list of targets (from enabled provider models), move controls, duplicate prevention, and a ten-target limit.
- Strategy selector: `sequential` always starts at target one; `round_robin` rotates the leading target while preserving the rest as ordered fallbacks.
...
- Empty state includes a one-click `never-stop` template built from up to three enabled models.
