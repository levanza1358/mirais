...
Card grid of **provider presets** (1-col → 4-col on xl), driven by a static catalog (`dashboard/src/providerCatalog.ts`): OpenAI (Codex), Anthropic, BlackBoxAI, GitHub Copilot, DeepSeek, xAI, GLM, and Custom. Each card:
```
┌──────────────────────────────────────┐
...
- Playback state is saved locally. Refreshing resumes the current song and position **only** when it was playing before the refresh; paused or stopped sessions remain paused.
- Music playback is single-tab: the first dashboard tab holds a short renewable local lease. Additional tabs do not hydrate or control a second music player while that lease is active.
