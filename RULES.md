...
- **R1.3** Dashboard routes under `/api/*` are intentionally passwordless. Protect exposed instances with a reverse proxy, firewall, VPN, or private network. Every `/v1/*` route requires gateway-key middleware. No exceptions.
