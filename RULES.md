...
- **R1.3** The dashboard password guards **only** the dashboard: every `/api/*` route except `/api/auth/*` and `/api/health` requires a valid session cookie. It is **on by default** (default password `12345678`, stored as a hash — operators must change it) and can be turned off from Settings → General. Session lifetime is operator-configurable (default `SESSION_TTL_HOURS`, up to 30 days with "remember"), so a page refresh never re-prompts inside the window. Network controls (reverse proxy, firewall, VPN, private network) remain the outer layer and are never replaced by the password. Gateway traffic is unaffected: every `/v1/*` route requires gateway-key middleware **unless `MIRAIS_AUTH_REQUIRED=off`** is set, in which case the operator accepts that the listener is reachable only from a trusted network (loopback bind / private LAN) and lets anonymous traffic through. No other exceptions.
