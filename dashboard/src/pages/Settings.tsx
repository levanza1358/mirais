...
function BackupSection() {
  ...
  const restore = useMutation({
    ...
    onSuccess: (data) => {
      ...
      toast(`${data.imported} accounts restored${data.skipped ? `, ${data.skipped} already exist` : ""}`);
    },
    onError: (e) => toast(e.message, "error"),
  });
  ...
  return (
    <Card>
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-medium">Account backup & restore</h3>
          </div>
          <p className="mt-1 text-xs text-text-muted">Exports provider accounts and their credentials as JSON. It does not include gateway keys, models, settings, logs, or usage.</p>
        </div>
        <div className="flex gap-2">
          <Button size="sm" onClick={() => create.mutate()} loading={create.isPending}><Download size={14} /> Export accounts</Button>
          <Button size="sm" variant="outline" onClick={() => fileRef.current?.click()} loading={upload.isPending}><Upload size={14} /> Upload</Button>
          <input ref={fileRef} type="file" accept=".json,application/json" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload.mutate(file); e.currentTarget.value = ""; }} />
        </div>
      </div>
      ...
      <Modal open={!!restoreId} onClose={() => setRestoreId(null)} title="Restore accounts">
        <p className="mb-5 text-sm text-text-muted">Accounts are added to matching providers. Existing accounts with the same API key are skipped. Nothing else is changed.</p>
        <div className="flex justify-end gap-2">
          <Button variant="outline" onClick={() => setRestoreId(null)} disabled={restore.isPending}>Cancel</Button>
          <Button onClick={startRestore} loading={restore.isPending}>Restore accounts</Button>
        </div>
      </Modal>
    </Card>
  );
